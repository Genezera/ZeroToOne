...
test('readiness audit consolida invariantes e mantém reports privados como limitação honesta', () => {
  ...
  writeFileSync(path.join(root, '.github', 'workflows', 'bugbounty-metrics.yml'), workflow, 'utf8');
...
test('readiness cloud-primary passa ao doctor somente os requisitos realmente obrigatórios', () => {
  ...
  closeDb(db);
  const workflow = 'on:\n  schedule:\n  workflow_dispatch:\npermissions:\n  contents: write\nconcurrency:\n  group: zerotoone-bugbounty-writer\n  cancel-in-progress: false\nsteps:\n  - uses: actions/checkout@' + 'a'.repeat(40) + '\n';
  for (const name of ['bugbounty-scan.yml', 'bugbounty-report-sync.yml', 'bugbounty-change-monitor.yml', 'bugbounty-target-discovery.yml', 'bugbounty-evidence.yml', 'bugbounty-metrics.yml']) {
