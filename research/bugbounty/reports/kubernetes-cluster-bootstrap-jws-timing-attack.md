...
## Category / Severity
Observable Timing Discrepancy (CWE-208) — specifically, a secret-derived cryptographic MAC compared with a non-constant-time operator, which is a narrower and more security-relevant claim than "some timing difference exists somewhere." I'm not self-assigning a severity label — the real question for triage is how much practical weight to give this. A single `kubeadm join` attempt gives an attacker exactly one measurement (see "Confirmed call chain" point 4); a repeat mechanism does exist across separate join events that reuse the same bootstrap token, which Kubernetes' own documentation confirms is a real, if discouraged, practice (point 5) — but I have not empirically measured timing signal over real network conditions across independent events, a harder problem than this report's local PoC addresses. That gap between "a repeat mechanism exists" and "I've shown it's exploitable over a network" is real and I'd rather state it than have triage discover it. For context, not as a self-assigned score: this program's own `severity-ratings.md` rates the closest documented analog to the full attack pattern here — a service masquerading as the API server to man-in-the-middle it — as High, not Critical, and separately defines "Adjacent Network Access" (the network position this attack requires, versus an arbitrary remote anonymous location) as downgrading Critical to High by default; a narrower, unproven variant of that same pattern would reasonably sit at or below that ceiling, not above it.
...
## Summary
`DetachedTokenIsValid` verifies a bootstrap token's JWS signature (HS256, i.e. HMAC-SHA256, a symmetric, secret-derived MAC — 43 base64url characters with no padding) by comparing the computed signature against the received one with Go's plain `==` string operator, whose execution time depends on the position of the first differing byte. The security property this breaks is specific: **the comparison's execution time becomes correlated with how many leading bytes of a secret-derived MAC a candidate got right** — e.g. a candidate matching only the first byte (`A???...` vs the correct `AXYZ...`) returns faster than one matching the first 42 of 43 bytes (`AXYZ...?` vs `AXYZ...Z`). That correlation is exactly what `crypto/hmac.Equal` (Go's own standard-library function for this) exists to eliminate. I built an isolated benchmark reproducing only this exact primitive and confirmed, with real statistical significance and reproduced independently three times, that this correlation is measurable in this specific comparison — not a theoretical concern. **What this does not establish on its own is that a remote attacker can actually observe that timing signal over a real network.** I did find a real repeat mechanism — multiple nodes reusing one bootstrap token, a documented, if discouraged, kubeadm practice (see "Confirmed call chain" point 5) — but I have not measured timing signal across independent network events, a harder problem than the same-process local benchmark in this report. See "Impact" for the full breakdown of what is and isn't shown.
...
5. Separately from any retry within a single invocation, a repeat mechanism does exist at the multi-node level: `kubernetes/website`'s own bootstrap-token documentation states that reusing the same token across multiple clients is a real, if discouraged, practice, and warns specifically about this signature-verification mechanism: "Any party with a bootstrapping token can create a valid signature for that token. When using ConfigMap signing it's discouraged to share the same token with many clients, since a compromised client can potentially man-in-the-middle another client relying on the signature to bootstrap TLS trust." This matches ordinary kubeadm usage: `kubeadm init` prints one join command carrying one token, and the common onboarding path for multiple worker nodes is running that same command — same token — on each one until it expires (24 hours by default) or is rotated. Under that real, documented pattern, each additional node's join is a separate, independent invocation of `DetachedTokenIsValid` against the identical secret — a repeat mechanism at the fleet-provisioning level, distinct from (and not dependent on) any retry loop inside one client's own call pattern.

## Prerequisites
An actor positioned to intercept and respond to the insecure discovery request a node sends while running `kubeadm join` with a bootstrap token — concretely, a network man-in-the-middle position between the joining node and the real control-plane endpoint, active during the token's validity window (24 hours by default, operator-configurable). This does not require possessing the token secret itself — that is precisely what the attack targets. Holding that position for the full window is worth more to the attacker than a brief one, since it's what would let them observe multiple join events if the same token is reused across nodes (see "Confirmed call chain" point 5) — though whether any given token actually gets reused, and how many times, is an operational choice this report cannot verify from the code alone. I have not verified whether any particular cluster's network topology makes this position easy or hard to obtain; that's deployment-specific.
...
3. Because the comparison in step 1 of "Confirmed call chain" is not constant-time, its execution time is, in principle, correlated with how much of the attacker's candidate signature matches — information a properly constant-time comparison would not leak. Within one join attempt this gives the attacker exactly one timed observation (point 4 of "Confirmed call chain"); further observations would come from separate join events that reuse the same token (point 5), a real but considerably harder-to-measure setting than a local benchmark. I have not demonstrated that the resulting signal is actually recoverable over real network conditions (see "Impact"); I'm naming that gap explicitly rather than asserting a working oracle that I have not demonstrated.

## Current vs. expected result
- **Current:** `DetachedTokenIsValid` compares the signature with `==`, whose execution time depends on the position of the first differing byte.
...
- **No repeatable interaction was found within a single join attempt.** Verification runs exactly once per `kubeadm join` invocation, outside any retry loop — the retry logic that exists (`getClusterInfo`) only re-fetches the ConfigMap until a JWS key is *present*, it never re-invokes `DetachedTokenIsValid` (point 4 of "Confirmed call chain"). **A repeat mechanism does exist at the fleet level, though**: Kubernetes' own documentation confirms that reusing one bootstrap token across multiple nodes is a real, if discouraged, practice, and separately warns about MITM risk on this exact mechanism (point 5) — each additional node joining with the same still-valid token gives a persistently-positioned attacker one more independent, real invocation to measure. I have not demonstrated this empirically: aggregating a timing signal across genuinely separate network events, rather than a tight local loop, is a substantially harder measurement problem than this report's PoC addresses, and I have no multi-node cluster to test it against.
...
```
`hmac.Equal` is the right choice specifically because both operands are HMAC-derived values — it expresses the intended cryptographic comparison semantics directly, rather than reaching for a generic constant-time-compare utility. `crypto/hmac.Equal` is already a transitive dependency of this file (`ComputeDetachedSignature` uses `go-jose`, which itself uses HMAC), so this adds no new dependency — only an added `crypto/hmac` import if not already present, and swaps one operator for one existing standard-library function.
