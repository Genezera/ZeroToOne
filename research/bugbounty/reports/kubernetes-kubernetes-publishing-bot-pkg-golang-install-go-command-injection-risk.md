...
- [ ] **Gate anti-duplicate bloqueado** — `InsecureSkipVerify:true` é design original de 2018, não uma regressão recente. A idade foi confirmada via git real, mas isso aumenta o risco de já existir report privado e não prova novidade. Não enviar sem uma regressão recente verificável.
...
- Classificação registrada: **longstanding_exposure** — evidência de idade, não de novidade
- Evidência de idade: commit real `edcff13f8546ec0db2ed86c248e94fca7e28fc7e` ("allow to fetch rules from URL", Michal Fojtik, 2018-02-13T13:06:34+01:00, commit assinado PGP verificado pelo GitHub) — **3125 dias** confirmados via `verify-longstanding-exposure`, ainda ancestral de `origin/HEAD`
- Gate atual: **BLOCK** — o modo anti-duplicate exige `noveltyStatus=regression`; exposição antiga aumenta o risco de duplicata e não prova novidade

> Correção metodológica de 04/09/2026: este rascunho chegou a ser marcado `human_ready` por uma exceção que aceitava longa exposição como novidade. A premissa era invertida e foi removida. O estado histórico pode permanecer no ledger, mas o preflight atual bloqueia o envio.
