...
This is the same defect class already found and fixed-in-report on this program's EVM counterpart (`evm-gateway-contracts::Withdrawals.sol`, missing `notDenylisted` modifier on `initiateWithdrawal`/`withdraw`) — here it is the same design/implementation gap, independently present in the Solana sibling program, confirmed by direct code reading (not inference by analogy).
