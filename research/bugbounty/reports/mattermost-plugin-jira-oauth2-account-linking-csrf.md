...
>
> **Nota de confiança sobre o achado em si (continua válida, independente do bloqueio acima):** tecnicamente este é o achado mais sólido encontrado nesta missão até agora. Diferente de outros rascunhos desta pasta, aqui NÃO há incerteza técnica: alcançabilidade, escopo, elegibilidade de recompensa e o próprio bug foram confirmados por **duas verificações adversariais independentes** (4 agentes céticos, depois uma sessão dirigida por humano que clonou o repositório de novo do zero e leu cada linha pessoalmente) e por uma **PoC real, executada, contra a função vulnerável verdadeira** (não uma reimplementação) — ver `research/bugbounty/mattermost-plugin-jira/evidence/oauth2-account-linking-csrf/` pro teste Go completo e a saída real de `go test`. Quanto a duplicata: NÃO dá pra dizer "alta chance de não ser" — busca pública nunca prova ausência de report privado (limitação estrutural da HackerOne), e o histórico real desta missão até agora é 5 relatórios enviados, 5 fechados sem pagamento (4 duplicata, 1 duplicata-de-informativo). O que dá pra dizer com honestidade: não achei cobertura pública deste caminho específico, e a classe de bug (ler uma assimetria entre duas rotas parecidas) é menos "óbvia" que os padrões batidos dos relatórios anteriores — chance melhor que a média, não garantida. Calibrei a severidade como Medium-High (não Critical) porque a exploração exige um passo de engenharia social e a página de confirmação final expõe visivelmente a inconsistência para a vítima — ambos os pontos já estão declarados no corpo do relatório abaixo, não escondidos. Decisão de enviar (e de severidade final), quando o programa reabrir, é sua.
