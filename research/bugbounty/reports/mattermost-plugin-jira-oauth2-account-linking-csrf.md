> **🛑 NÃO ENVIAR AGORA — o programa Mattermost não está aceitando submissões no momento** (você mesmo viu o banner "Mattermost is not accepting submissions at this time" na página real; confirmei ao vivo via API do HackerOne: `submissionState="disabled"`, `state="public_mode"`, `offersBounties=true` — o programa continua público e paga bounty normalmente, só não recebe report novo agora). Isso não tem nada a ver com a qualidade do achado; é bloqueio administrativo do lado do Mattermost. Adicionei esse bloqueio ao `program-policy.json` e corrigi uma lacuna real do pipeline (esse estado do programa nunca era checado em lugar nenhum antes — só descoberto porque você mandou o screenshot). Revise de novo antes de enviar: rode `node -e "import('./system/bugbounty-scanner/h1-api.mjs').then(m=>m.getProgram('mattermost').then(p=>console.log(p)))"` da raiz do repo, ou reabra a página — só envie quando `submissionState` voltar a `"open"`.
>
> **Estado da evidência:** o defeito de vinculação foi reproduzido na função real e na persistência real do plugin. As chamadas externas à Atlassian foram simuladas com `httpmock`; portanto isto ainda não é uma demonstração browser-to-browser nem prova de um token Jira real sendo usado pelo atacante. A busca pública não encontrou este caminho específico, mas relatórios privados permanecem invisíveis e duplicata não pode ser descartada. Severidade recomendada: **Medium**, condicionada à política do programa e, idealmente, a uma reprodução end-to-end com duas contas controladas pelo pesquisador quando submissões voltarem a ser aceitas.
...
## Category / Severity
CWE-352 (Cross-Site Request Forgery), specifically missing binding between the OAuth2 `state` owner and the Mattermost session completing the flow. Recommended severity: **Medium**. Exploitation requires the victim to open an attacker-supplied authorization link and approve Atlassian consent, and the final confirmation page visibly shows the mismatched Mattermost/Jira pairing with a Disconnect action.
...
- Original PoC captured at `f65d8b9750c4cafbe4fbacd7aa36ca96e19f6b15`; independently revalidated against default-branch commit `0e5b7423fe406e69ef668d301ecfbc7fefc8f710` (2026-09-10)
...
## Expected exploitation path (not yet executed end-to-end)
...
6. Based on the command-to-connection call chain, the attacker's later `/jira` commands would load that stored connection. This final use of a real victim token has not yet been executed in the supplied PoC.
...
## Proof of concept
I wrote a Go test that calls the actual, unmodified `httpOAuth2Complete` and `GetUserConnectURL` functions directly against a fresh clone — no plugin logic was reimplemented. The outbound HTTPS calls for token exchange, accessible resources, and Jira identity are simulated using `jarcoal/httpmock`. The PoC proves the handler's identity decision and persistence behavior, not issuance or later use of a real Atlassian token.
...
=== RUN   TestZeroToOne_OAuth2CompleteIgnoresSessionIdentity
    ...
    zz_zerotoone_oauth2_csrf_test.go:159: RESULT: attacker Mattermost id "attacker0000000000000000000" is linked to a simulated victim Jira identity, while the different Mattermost user id sent in the completing request was never queried.
...
- The attacker's connection holds the non-nil token returned by the mocked exchange.
- The attacker's connection's `AccountID` equals the simulated victim Jira account id.
...
## Impact
The reproduced code path persists the Jira token and identity returned from the OAuth exchange under the Mattermost user id embedded in the attacker's `state`, ignoring a different user id on the completing request. If Atlassian preserves the supplied `state` through victim consent as expected, subsequent `/jira` commands by the attacker would load the victim-authorized connection and act with its Jira permissions. The supplied PoC does not yet execute that full external chain. Exploitation also requires victim interaction and presents a visible mismatched-account confirmation, supporting a Medium rather than High/Critical estimate.
