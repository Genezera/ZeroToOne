# Incorrect Argument Order Makes the Published GraphQL `@requires` Authorization Directive Reject Every Protected Field Resolution (`@kiwicom/iam`)
...
- Confirmed identical at the current default-branch HEAD (commit `a5ca3d73fb83fab4dfbb347bf6a994da175b1f8e`) and at the latest published release tag `v2.3.0` — the version real consumers install from npm. This is one of the package's documented GraphQL usage patterns (the SDL/directive approach; the README also documents calling `isUserAuthorized` directly), not an unused code path.
...
I also verified, using the real `graphql` execution engine (not asserted from memory), a detail that affects how far this blast radius reaches within a single request: when a resolver throws and the field is declared Non-Null in the schema (as the README's own `paymentCard: String @requires(...)` example is not, but many real fields protected this way could be), GraphQL's own null-propagation rules nullify the *entire* response, not just that field — I confirmed this with a minimal schema and got back `{"errors": [...], "data": null}` even when an unrelated, unprotected field was requested alongside it in the same query. Whether any specific field this directive protects is declared Non-Null in a real deployment is not something I can verify without a real schema to look at, but the mechanism itself is real GraphQL behavior, not speculation.
