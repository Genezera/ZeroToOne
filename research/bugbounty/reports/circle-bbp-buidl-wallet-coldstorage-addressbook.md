...
**Deployment status:** I have not found a confirmed mainnet deployment address for a live MSCA account with this module installed — `buidl-wallet-contracts` is the published, general-purpose smart-contract-account framework Circle ships (used to build BUIDL and other Circle wallet products), not itself a single deployed instance. This report is about the framework code as published and consumable by any integrator today, the same basis as the existing `evm-gateway-contracts`/`solana-gateway-contracts` findings before their deployment status was separately confirmed.
